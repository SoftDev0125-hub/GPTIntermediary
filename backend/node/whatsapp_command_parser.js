...
function parseWhatsAppCommand(text) {
  ...
  // "Send <recipient> a message on WhatsApp [content]" — recipient first, then content
  const reSendRecipientThenMessage = /^\s*send\s+(.+?)\s+a\s+message\s+(?:on|via)\s+whatsapp\s*(?:\s*[:\-]\s*|\s+)([\s\S]+)\s*$/i;

  let m;
  if ((m = s.match(reSendRecipientThenMessage))) {
    return { platform: 'whatsapp', action: 'send', recipient: m[1].trim().replace(/^["']|["']$/g, '').trim(), body: m[2].trim() };
  }
